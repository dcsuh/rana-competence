...
```{r}
comm_summ <- data %>% dplyr::select(WetAltID, Month.1, cc, Month, AB2:AB42, Prevalence) %>% 
...
richness <- tibble()
for (i in 1:length(rownames(presence_mat))){
  richness[i,1] <- rownames(presence_mat)[i]
  richness[i,2] <- sum(presence_mat[i,])
}
richness %<>% rename(siteID = 1, richness = 2)
```
...
```{r}
richness_cc <- full_join(comm_summ, richness_mat, by = "siteID")
