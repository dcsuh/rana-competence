...
figure_1 + theme(title = "Original fig w/ lines for R0==1")
```
...
```{r}
(comp_fig | size_fig)/(half_fig | combined_fig) + theme(title = "heatmap of difference between treatment and reference")
```
...
```{r}
(comp_fig | size_fig)/(half_fig | combined_fig) + theme("filled contours of diff between treatment and reference")
```
