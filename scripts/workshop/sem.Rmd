...
```{r}
potential <- comm_data %>% mutate(log_cc = log10(cc), log_size = log10(size)) %>% select(cc, log_cc, size, log_size, MeanWaterTempPredC, Prevalence) %>% drop_na() %>% filter(MeanWaterTempPredC > 0)
cor(potential[,c(2,4,5)])
```

```{r}
potential_formula <- "potential =~ log_cc + log_size + MeanWaterTempPredC"
...
```{r}
potential_formula_1 <- "
potential =~ log_cc + log_size + MeanWaterTempPredC
