...
```{r}
cc_temp <- glm(log10(cc) ~ MeanWaterTempPredC, data = comm_data)
size_temp <- glm(log10(size) ~ MeanWaterTempPredC, data = comm_data)
prev_cc <- glm(Prevalence ~ log10(cc) + MeanWaterTempPredC + log10(size), data = comm_data)
