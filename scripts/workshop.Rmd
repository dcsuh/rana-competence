...
Normalized to have range 0 to 1
```{r}
...
bc_cc_df %<>% mutate(diff = abs(bc_dist-cc_norm))

ggplot(bc_cc_df, aes(x = col, y = row, fill = diff)) + geom_tile()

bc_cc_df %>% filter(bc_dist!=1) %>% ggplot(.,aes(x=cc_norm, y=bc_dist)) + geom_point() + geom_smooth(method = "lm")
```
...
The manhattan distance is equivalent to bray-curtis distance
